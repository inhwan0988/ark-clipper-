...
export async function POST(req: Request) {
  ...
  try {
    ...
    const results = await suggestEmojis(transcript.segments, apiKey);

    const map = new Map<number, string>();
    for (const r of results) map.set(r.segmentId, r.emoji);
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'emoji 매칭 실패' },
      { status: 500 },
    );
  }
}
